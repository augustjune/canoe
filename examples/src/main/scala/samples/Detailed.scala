...
object Detailed extends IOApp {
  ...
  def pizzaOrders[F[_]: TelegramClient](pizzaPlace: PizzaPlace[F]): Scenario[F, Unit] =
    for {
      chat   <- Scenario.expect(command("pizza").chat)  // Defines the start of the scenario with '/pizza' command
      ...
      order  <- Scenario.expect(text)                   // Defines the next step of the scenario to be a text message
